...
export default class extends React.Component {
	 ...
	 */
	onShowSubmitModal() {
		this.getFormRef().current.validateFields().then(() => {
			this.setState({
	...
	render() {
		...
		return <div className={FormStyles['edi-excel-generator']}>
			<Form ref={ref} onFinish={ this.handleFileGenerate.bind(this) } initialValues={ this.getFormDefaultValues() } {...layout} className="generator-form">
				...
				{this.getFormItems()}
				<Form.Item wrapperCol={{ span: 12, offset: 5 }}>
					<Button loading={ this.state.isGenerating } type="primary" title="生成当前文档文件" htmlType="submit">生成</Button>
					&nbsp;&nbsp;
					<Button onClick={this.onClearForm.bind(this)} title="清空当前表单内容" htmlType="button">清空</Button>
					&nbsp;&nbsp;
					<Button onClick={this.onShowSubmitModal.bind(this)} title="直接提交当前数据" htmlType="button">提交</Button>
				</Form.Item>
			</Form>
			<Modal visible={ this.state.showSubmitConfirm }
						 title="确认提交当前数据?"
				...
				<Form.Item style={ {marginBottom: 5} } label="文件名称">
					<Input size="small" value={ this.state.submittingTitle } onChange={ e => this.setState({submittingTitle: e.target.value}) } />
				</Form.Item>
