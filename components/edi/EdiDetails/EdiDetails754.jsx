...
export default ({file}) => {
	const shipFrom = [file.from_code, file.from_street, file.from_city, file.from_state, file.from_zipcode, file.from_country].filter(Boolean).join(', ');
	const shipTo = [file.receiver, file.to_street, file.to_city, file.to_state, file.to_zipcode, file.to_country].filter(Boolean).join(', ');

	return <Descriptions size="middle" layout="vertical" title="754信息" bordered>
