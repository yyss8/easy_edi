...
export default async (req, res) => {
	switch (req.method.toLowerCase()) {
		...
		case 'post':
			...
			const updatingProducts = products.filter(product => Boolean(product.product_id) && prevProducts.indexOf(product.product_id) > -1);
			const deletingProducts = prevProducts.filter(productId => !products.find(product => product.product_id === productId));
