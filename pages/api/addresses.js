	...
	switch (req.method.toLowerCase()) {
		case 'get':
			...
			if (typeof req.query.keyword !== 'undefined') {
				...
				query.whereRaw('address_title=?', keyword);
			}
		...
		case 'post':
			...
			const updatingAddresses = addresses.filter(address => Boolean(address.address_id) && prevAddresses.indexOf(address.address_id) > -1);
			const deletingAddresses = prevAddresses.filter(addressId => !addresses.find(address => address.address_id === addressId));
